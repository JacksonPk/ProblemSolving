...
//var room: [[String]] = [[String]].init(repeating: [String].init(repeating: " ", count: 1000), count: 1000)
    ...
    for i in 0..<n {
        ...
        if shouldCallFunc != false {
            for j in 0..<m {
                ...
                switch room[i][j] {
                case "@":
                    if (i == 0 || i == n-1 || j == 0 || j == m-1) { shouldCallFunc = false; break }
                    pos.append([i,j])
                case "*":
                    fires.append([i,j])
                default:
        ...
        while ps<pe && !isEnd {
            ...
            for i in fs..<fe {
                ...
                for d in 0..<4 {
                    let nR = cR+dR[d]
                    let nC = cC+dC[d]
                    if (nR >= 0 && nR < n && nC >= 0 && nC < m) && (room[nR][nC] == "." || room[nR][nC] == "@") {
                        room[nR][nC] = "*"
                        fires.append([nR,nC])
                        fdiff += 1
                    }
